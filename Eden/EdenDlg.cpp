...
UINT CEdenDlg::taskbarCreated = 0;

CEdenDlg::CEdenDlg(CWnd* pParent /*=NULL*/)
	: CDialogEx(CEdenDlg::IDD, pParent)
	, log(_T(""))
	, statusLog(_T(""))
	, pgInfo(_T(""))
{
...
void CEdenDlg::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CEdenDlg, CDialogEx)
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_WM_CREATE()
	ON_WM_SYSCOMMAND()
	...
	ON_WM_TIMER()
	ON_WM_SIZE()
	ON_REGISTERED_MESSAGE(CEdenDlg::taskbarCreated, OnTaskbarCreated)
	ON_CBN_SELCHANGE(IDC_COMBO_TUNER, &CEdenDlg::OnCbnSelchangeComboTuner)
	ON_CBN_SELCHANGE(IDC_COMBO_SERVICE, &CEdenDlg::OnCbnSelchangeComboService)
	ON_BN_CLICKED(IDC_BUTTON_SET, &CEdenDlg::OnBnClickedButtonSet)
	ON_BN_CLICKED(IDC_BUTTON_CHSCAN, &CEdenDlg::OnBnClickedButtonChscan)
	ON_BN_CLICKED(IDC_BUTTON_EPG, &CEdenDlg::OnBnClickedButtonEpg)
	ON_BN_CLICKED(IDC_BUTTON_REC, &CEdenDlg::OnBnClickedButtonRec)
	ON_BN_CLICKED(IDC_BUTTON_CANCEL, &CEdenDlg::OnBnClickedButtonCancel)
	ON_BN_CLICKED(IDC_BUTTON_VIEW, &CEdenDlg::OnBnClickedButtonView)
	ON_BN_CLICKED(IDC_CHECK_UDP, &CEdenDlg::OnBnClickedCheckUdp)
	ON_BN_CLICKED(IDC_CHECK_TCP, &CEdenDlg::OnBnClickedCheckTcp)
	ON_BN_CLICKED(IDC_CHECK_REC_SET, &CEdenDlg::OnBnClickedCheckRecSet)
	ON_BN_CLICKED(IDC_CHECK_NEXTPG, &CEdenDlg::OnBnClickedCheckNextpg)
	ON_BN_CLICKED(IDC_BUTTON_TIMESHIFT, &CEdenDlg::OnBnClickedButtonTimeshift)
	ON_WM_QUERYENDSESSION()
	ON_WM_ENDSESSION()
END_MESSAGE_MAP()


// CEdenDlg メッセージ ハンドラー
void CEdenDlg::SetInitBon(CString bonFile)
{
...
//  システムがこの関数を呼び出します。
HCURSOR CEdenDlg::OnQueryDragIcon()
{
...
BOOL CEdenDlg::PreTranslateMessage(MSG* pMsg)
{
...
int CEdenDlg::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
...
void CEdenDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
...
void CEdenDlg::OnTimer(UINT_PTR nIDEvent)
{
	// TODO: ここにメッセージ ハンドラー コードを追加するか、既定の処理を呼び出します。
	switch(nIDEvent){
		case TIMER_INIT_DLG:
...
void CEdenDlg::OnSize(UINT nType, int cx, int cy)
{
...
LRESULT CEdenDlg::WindowProc(UINT message, WPARAM wParam, LPARAM lParam)
{
	// TODO: ここに特定なコードを追加するか、もしくは基本クラスを呼び出してください。
	switch(message){
...
BOOL CEdenDlg::AddTaskBar(HWND wnd, UINT msg, UINT id, HICON icon, CString tips)
{ 
	BOOL ret=TRUE;
	NOTIFYICONDATA data;
...
BOOL CEdenDlg::DeleteTaskBar(HWND wnd, UINT id)
{ 
	BOOL ret=TRUE; 
	NOTIFYICONDATA data; 
	ZeroMemory(&data, sizeof(NOTIFYICONDATA));
...
LRESULT CEdenDlg::OnTaskbarCreated(WPARAM, LPARAM)
{
...
void CEdenDlg::BtnUpdate(DWORD guiMode)
{
	switch(guiMode){
		case GUI_NORMAL:
...
void CEdenDlg::OnCbnSelchangeComboTuner()
{
...
void CEdenDlg::OnCbnSelchangeComboService()
{
	// TODO: ここにコントロール通知ハンドラー コードを追加します。
	int sel = this->combService.GetCurSel();
	if( sel != CB_ERR ){
...
void CEdenDlg::OnBnClickedButtonSet()
{
	// TODO: ここにコントロール通知ハンドラー コードを追加します。
	CSettingDlg setDlg;
...
void CEdenDlg::ReloadServiceList(BOOL ini)
{
...
DWORD CEdenDlg::SelectBonDriver(LPCWSTR fileName, BOOL ini)
{
	this->main.CloseBonDriver();
...
DWORD CEdenDlg::SelectService(WORD ONID, WORD TSID, WORD SID)
{
	DWORD err = this->main.SetCh(ONID, TSID, SID);
	return err;
}

DWORD CEdenDlg::SelectService(WORD ONID, WORD TSID, WORD SID,	DWORD space, DWORD ch)
{
	DWORD err = this->main.SetCh(ONID, TSID, SID, space, ch);
	return err;
}

void CEdenDlg::OnBnClickedButtonChscan()
{
	// TODO: ここにコントロール通知ハンドラー コードを追加します。
...
void CEdenDlg::OnBnClickedButtonEpg()
{
	// TODO: ここにコントロール通知ハンドラー コードを追加します。
...
void CEdenDlg::OnBnClickedButtonRec()
{
	// TODO: ここにコントロール通知ハンドラー コードを追加します。
...
void CEdenDlg::OnBnClickedButtonCancel()
{
	// TODO: ここにコントロール通知ハンドラー コードを追加します。
	if( this->main.IsRec() == TRUE ){
...
void CEdenDlg::OnBnClickedButtonView()
{
	// TODO: ここにコントロール通知ハンドラー コードを追加します。
...
void CEdenDlg::OnBnClickedCheckUdp()
{
	// TODO: ここにコントロール通知ハンドラー コードを追加します。
	this->main.SendUDP(this->chkUDP.GetCheck());
}
...
void CEdenDlg::OnBnClickedCheckTcp()
{
	// TODO: ここにコントロール通知ハンドラー コードを追加します。
	this->main.SendTCP(this->chkTCP.GetCheck());
}
...
void CEdenDlg::OnBnClickedCheckRecSet()
{
	// TODO: ここにコントロール通知ハンドラー コードを追加します。
...
void CEdenDlg::OnBnClickedCheckNextpg()
{
	// TODO: ここにコントロール通知ハンドラー コードを追加します。
	CString info = L"";
	this->main.GetEpgInfo(this->btnPgNext.GetCheck(), &info);
...
void CEdenDlg::OnBnClickedButtonTimeshift()
{
	// TODO: ここにコントロール通知ハンドラー コードを追加します。
...
BOOL CEdenDlg::OnQueryEndSession()
{
...
void CEdenDlg::OnEndSession(BOOL bEnding)
{
