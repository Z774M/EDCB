...
void CEdenMain::SetHwnd(HWND wnd)
{
	this->msgWnd = wnd;
}

//設定を行う
void CEdenMain::ReloadSetting()
{
...
//戻り値：
// エラーコード
DWORD CEdenMain::CloseBonDriver()
{
	this->currentBonDriver = L"";
...
DWORD CEdenMain::GetServiceList(
...
BOOL CEdenMain::GetOpenBonDriver(
...
//戻り値：
// TRUE（変更中）、FALSE（完了）
BOOL CEdenMain::IsChChanging(BOOL* chChgErr)
{
	return this->bonCtrl.IsChChanging(chChgErr);
}

void CEdenMain::SetSID(
	WORD SID
	)
{
	this->lastSID = SID;
...
//UDP送信の設定数を取得
DWORD CEdenMain::GetCountUDP()
{
	return this->udpCount;
}

//TCP送信の設定数を取得
DWORD CEdenMain::GetCountTCP()
{
	return this->tcpCount;
}

//UDPの送信中先一覧取得
BOOL CEdenMain::GetSendUDPList(
...
//戻り値：
// シグナルレベル
float CEdenMain::GetSignalLevel()
{
	return this->bonCtrl.GetSignalLevel();
}

//エラーカウントをクリアする
void CEdenMain::ClearErrCount(
	)
{
	if( this->nwCtrlID != 0 ){
		this->bonCtrl.ClearErrCount(this->nwCtrlID);
...
BOOL CEdenMain::IsRec()
{
	return this->bonCtrl.IsRec();
}

//予約録画を停止する
void CEdenMain::StopReserveRec()
{
...
DWORD CEdenMain::GetChScanStatus(
	DWORD* space,
	DWORD* ch,
	wstring* chName,
	DWORD* chkNum,
...
//引数：
// info			[OUT]取得中のサービス
DWORD CEdenMain::GetEpgCapStatus(
...
void CEdenMain::StopServer()
{
	this->pipeServer.StopServer();
}

void CEdenMain::StartTimeShift()
{
	wstring saveFile = L"";
	DWORD ctrlID = 0;
	if( this->recCtrlID != 0 ){
...
BOOL CEdenMain::GetViewStatusInfo(
	float* signal,
	DWORD* space,
	DWORD* ch,
	ULONGLONG* drop,
...
int CALLBACK CEdenMain::CtrlCmdCallback(void* param, CMD_STREAM* cmdParam, CMD_STREAM* resParam)
{
	CEdenMain* sys = (CEdenMain*)param;
